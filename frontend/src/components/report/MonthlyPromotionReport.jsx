...
const MonthlyPromotionReport = ({ onBack }) => {
    ...
    const fetchPromotions = async (filterVal = 'all') => {
        setLoading(true);
        try {
            let data;
            if (filterVal === 'general') {
                data = await getPromotions(null, null, true);
            } else if (filterVal && filterVal !== 'all') {
                data = await getPromotions(filterVal);
            } else {
    ...
    const handleSubmit = async (e) => {
        ...
        if (selectedEventId) {
            ...
            if (selectedEvent && selectedEvent.datetime_start) {
                const eventStartDate = new Date(selectedEvent.datetime_start);
                
                const startLimit = new Date(eventStartDate);
                startLimit.setMonth(startLimit.getMonth() - 6);
                startLimit.setHours(0, 0, 0, 0);
                
                const endLimit = new Date(eventStartDate);
                endLimit.setHours(23, 59, 59, 999);
                
                const expenseTime = new Date(expenseDate).setHours(12, 0, 0, 0);
                
                if (expenseTime < startLimit || expenseTime > endLimit) {
                    const startStr = startLimit.toLocaleDateString();
                    const endStr = endLimit.toLocaleDateString();
                    alert(`無效時間！\n宣傳費的有效日期範圍為活動開始前6個月至活動開始時間：\n${startStr} 至 ${endStr}\n請重新選擇日期。`);
                    return;
                }
            }
        }
    ...
    const dateMin = (() => {
        if (!selectedEventForDate?.datetime_start) return '';
        const d = new Date(selectedEventForDate.datetime_start);
        d.setMonth(d.getMonth() - 6);
        return d.toISOString().split('T')[0];
    })();
    const dateMax = selectedEventForDate?.datetime_start
